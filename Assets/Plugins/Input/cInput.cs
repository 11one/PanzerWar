...
public class cInput : MonoBehaviour {
...
#endif
	public static bool MobileEnableMonitor = false;
	...
	private static void _SetAxisSensitivity(int hash, float sensitivity, string description = "") {
		...
		if (axis == -1) {
			...
			if (_FindKeyByHash(_makeAxis[axis, 1]) >= 0) {
	...
	private static void _LoadInputs() {
		...
		if (!_usePlayerPrefs) { return; }

		// make sure we have data saved to PlayerPrefs
	...
	#region _ConvertString2Key function
	...
	#endregion //_ConvertString2Key function
	...
	#region ShowMenu functions (DEPRECATED!)

	[Obsolete("cInput.ShowMenu has been deprecated. Please use cGUI.ToggleGUI() instead.")]
	...
	#endregion //ShowMenu functions (DEPRECATED!)
